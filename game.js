/* =========================================================
   ...
   - Uses UI overhaul HTML/CSS
   - Fix: null addEventListener crash
   - Fix: title screen tap works
   - Wardrobe, lore, achievements, dev menu, powers all active
   ========================================================= */
(() => {
  'use strict';
  ...
  const ART = {
    ...
    colors: {
      '1': '#ffd5a3', '2': '#ffee7a', '3': '#000', '4': '#fff',
      '5': '#ff3860', '6': '#000', '7': '#999', '8': '#aaf', '9': '#ccc'
    }
  };
  ...
  const SV = {
    ...
    progress: Store.get('sv_prog',{
      ach:{}, jumps:0, lastCheckpoint:0,
      beatBoss1:false, beatBoss2:false,
      endlessUnlocked:false, allTimeScore:0
    }),
  ...
  function init() {
    ...
    on(titleScreen, 'pointerdown', handleTitleStart);
    on(titleScreen, 'click', handleTitleStart);
    ...
    const updSet = () => {
      ...
      setText('music-toggle', `Music: ${SV.settings.music ? 'ON' : 'OFF'}`);
      setText('pause-music-btn', `Music: ${SV.settings.music ? 'ON' : 'OFF'}`);
      setText('sfx-toggle', `SFX: ${SV.settings.sfx ? 'ON' : 'OFF'}`);
      setText('pause-sfx-btn', `SFX: ${SV.settings.sfx ? 'ON' : 'OFF'}`);
  ...
  function startRun(lv) {
    ...
    ['home-screen','start-popup','death-popup','rpg-overlay']
      .forEach(id => { const el = qs('#' + id); if (el) el.classList.add('hidden'); });

    const gs = qs('#game-screen');
    ...
    if (nameDisplay) nameDisplay.textContent = SV.settings.playerName || "Hero";
  ...
  function update(dt) {
    ...
    SV.score += dt * 0.01;
    const scoreDisp = qs('#score-display');
    if (scoreDisp) scoreDisp.textContent = Math.floor(SV.score);

    if (SV.score > (SV.progress.allTimeScore || 0)) {
      SV.progress.allTimeScore = SV.score;
      Store.set('sv_prog', SV.progress);
    }
    const bestDisp = qs('#alltime-display');
    if (bestDisp) bestDisp.textContent = Math.floor(SV.progress.allTimeScore || 0);
  ...
  function initWardrobe() {
    ...
    const rebuildSkins = () => {
      ...
      SKINS.forEach(skin => {
        ...
        const unlocked = !!(have[skin.id] || have[skin.req]);
        ...
        const reqText = unlocked ? 'Unlocked' :
          'Unlock: ' + (ach ? ach.title : skin.req);
